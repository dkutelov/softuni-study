...
const userValidator = {
    validateRegister({ email, password, rePassword, fullName }) {
        let registerErrors = [
        ...
        if (fullName === '') {
            registerErrors.push('Full name can not be empty!');
        }
    ...
    validateLogin({ email, password }) {
        ...
        if (
            !password.match(ENGLISH_ALPHANUMERIC_PATTERN) ||
            password.length < 3
        ) {
            loginErrors.push(
        ...
        if (
            !email.match(ENGLISH_ALPHANUMERIC_EMAIL_PATTERN) ||
            email.length < 3
        ) {
            console.log(email);
            loginErrors.push(
